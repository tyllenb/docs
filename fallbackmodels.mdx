...
description: Providing security against downtime
---
...
<AccordionGroup>
  <Accordion title="Dynamic Pricing">
    The cost of the request is based on the model that successfully processes it. This information is included in the `model` attribute of the response body.
  </Accordion>

  <Accordion title="Fallback Model Pricing">
    If no fallback model is specified (`models` array contains only the primary model) but `route: "fallback"` is included, OpenRouter will automatically select an appropriate open-source model. This model will have a pricing structure close to or less than the primary model.
  </Accordion>
</AccordionGroup>
